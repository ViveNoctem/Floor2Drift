...
abstract class TestTaskDao extends BaseDao<TestTask> {
  ...
  Future<List<TestUser>> getAllUsers();

  @TestAnnotation()
  @Query('SELECT * FROM TaskTest WHERE id = :id LIMIT 1')
  Future<TestTask?> findTaskById(int id);

  /// Documentation method findTaskIdById
  @Query('SELECT id FROM TaskTest WHERE id = :id')
  ...
  @Query('SELECT DISTINCT COUNT(DISTINCT message) FROM TaskTest')
  Stream<int?> findUniqueMessagesCountAsStream();

  @Query('SELECT * FROM TaskTest WHERE status = :status')
  ...
  @Query('SELECT * FROM TaskTest WHERE id = :id')
  Future<TestTask?> whereId(int id);

  @Query('SELECT * FROM TaskTest WHERE id != :id')
  Future<List<TestTask>> whereNotEqual(int id);

  @Query('SELECT * FROM TaskTest WHERE timestamp != :timestamp')
  ...
  @Query('SELECT * FROM TaskTest WHERE id = :id AND isRead = :isRead')
  Future<List<TestTask>> whereAnd(int id, bool isRead);

  @Query('SELECT * FROM TaskTest WHERE id = :id OR isRead = :isRead')
  Future<List<TestTask>> whereOr(int id, bool isRead);

  @Query('SELECT * FROM TaskTest WHERE id = :id OR (isRead = :isRead AND status = :status)')
  Future<List<TestTask>> whereAndOr(int id, bool isRead, TaskStatus status);

  @Query('SELECT * FROM TaskTest WHERE isRead = :isRead AND (type IS NULL OR status = :status)')
  ...
  // region WHERE IN
  @Query('SELECT * FROM TaskTest WHERE id IN (:ids)')
  Future<List<TestTask>> whereIn(List<int> ids);

  @Query('SELECT * FROM TaskTest WHERE status IN (:status)')
  Future<List<TestTask>> whereInEnum(List<int> status);

  @Query('SELECT * FROM TaskTest WHERE id IN (5, 8, 2)')
  Future<List<TestTask>> whereIn582();

  @Query('SELECT * FROM TaskTest WHERE id NOT IN (:ids)')
  Future<List<TestTask>> whereNotIn(List<int> ids);

  @Query('SELECT * FROM TaskTest WHERE status NOT IN (:status)')
  Future<List<TestTask>> whereNotInEnum(List<int> status);

  @Query('SELECT * FROM TaskTest WHERE id NOT IN (7,9,1)')
  Future<List<TestTask>> whereNotIn791();

  @Query('SELECT * FROM TaskTest WHERE id = :id OR (isRead = :isRead AND status IN (:status))')
  ...
  @Query('SELECT status FROM TaskTest WHERE status = :status')
  ...
  @Query('SELECT * FROM TaskTest WHERE message LIKE :likeClause')
  ...
  @Query("DELETE FROM TaskTest WHERE id = :id")
  Future<void> deleteWhereId(int id);

  @Query("DELETE FROM TaskTest")
  ...
  @Query("UPDATE TaskTest SET message = :message WHERE id = :id")
  Future<int?> updateMessage(int id, String message);

  @Query("UPDATE TaskTest SET message = :message WHERE id IN (:ids)")
  Future<int?> updateMultipleMessages(List<int> ids, String message);

  // endregion

  // region BETWEEN
  @Query("SELECT * FROM TaskTest WHERE id BETWEEN :fromId  AND :toId")
  Future<List<TestTask>> betweenId(int fromId, int toId);

  @Query("SELECT * FROM TaskTest WHERE message NOT BETWEEN :fromMessage  AND :toMessage")
  ...
  @Query("SELECT * FROM TaskTest WHERE type NOT BETWEEN :fromType  AND :toType")
  ...
  @Query("SELECT * FROM TaskTest ORDER BY id")
  Future<List<TestTask>> orderById();

  @Query("SELECT * FROM TaskTest ORDER BY id ASC")
  Future<List<TestTask>> orderByIdAsc();

  @Query("SELECT * FROM TaskTest ORDER BY id DESC")
  Future<List<TestTask>> orderByIdDesc();

  @Query("SELECT * FROM TaskTest ORDER BY type DESC NULLS LAST")
  Future<List<TestTask>> orderByTypeDescNullsLast();

  @Query("SELECT * FROM TaskTest ORDER BY type DESC NULLS FIRST")
  Future<List<TestTask>> orderByTypeDescNullsFirst();

  @Query("SELECT * FROM TaskTest ORDER BY message Asc, id")
  Future<List<TestTask>> orderByMessageAscId();

  @Query("SELECT * FROM TaskTest ORDER BY message DESC, id DESC")
  Future<List<TestTask>> orderByMessageDescIdAsc();

  @Query("SELECT * FROM TaskTest WHERE id < :argument ORDER BY id DESC")
  Future<List<TestTask>> getOrderByWhere(int argument);
  // endregion

  // region subQuery
  @Query("SELECT * FROM TaskTest WHERE id IN (SELECT id FROM TaskTest WHERE id IN (:argument))")
